<?php
/* SVN FILE: $Id: shortcodes.php 178 2010-08-24 10:40:35Z nick@intellispire.com $*/
/**
*
* Copyright (c)2010 Nick Temple, Intellispire
*
...
* @copyright    2010 Intellispire/Nick Temple
* @version SVN: $Id: shortcodes.php 178 2010-08-24 10:40:35Z nick@intellispire.com $
*
*/
...
class plgSystemShortcodes extends JPlugin {
  ...
  var $default =  "<?php\ndefined('_JEXEC') or die('Restricted access');\n  function shortcode_test() {\n  return 'Hello shortcode world!';\n  }\n  add_shortcode('test', 'shortcode_test');\n";

  function __construct(& $subject, $config)
  {
    parent::__construct($subject, $config);

    $path = dirname(dirname(__FILE__)) . "/shortcodes";
    if (! file_exists($path)) {
      @mkdir($path);
      @copy(dirname(__FILE__) . "/index.html", "$path/index.html");
      @file_put_contents("$path/default.php", $this->default);
    }
    $this->loadShortcodes($path);
  }
  ...
  function loadShortcodes($base) {
    if ($handle = opendir($base)) {
      while (false !== ($file = readdir($handle))) {
        $ext = pathinfo($file, PATHINFO_EXTENSION);
        if (is_file("$base/$file") && ($ext == 'php')) {
          include("$base/$file");
        }
      }
      closedir($handle);
